package utils;

public class Const {
    public static final String VOLUME_BEGINNING = "VOLUME";
    public static final String BOOK_BEGINNING = "BOOK";
    public static final String CHAPTER_BEGINNING = "CHAPTER";


    public static final String VOLUME_SYSTEM_NAME = "volume";
    public static final String BOOK_SYSTEM_NAME = "book";
    public static final String CHAPTER_SYSTEM_NAME = "chapter";




    public static final String FILE_SUFFIX = ".txt";
    public static final String STATE_FILE_NAME = "state"+FILE_SUFFIX;


    public static final int COUNT_OF_BOSS_THREADS = 2;
    public static final int COUNT_OF_UNDER_BOSS_THREADS = 1;
    public static final int COUNT_OF_MASTER_THREADS = 1;
    public static final int COUNT_OF_FOREMAN_THREADS = 1;
    public static final int COUNT_OF_SLAVE_THREADS = 4;

}
